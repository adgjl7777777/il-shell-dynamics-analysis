...
FIG_DIR   = os.path.join(FIGURE_ROOT, "sensitivity")
...
for anion, T in PLOT_SYSTEMS:
    ...
    fig.suptitle(f"Bridging anion timeline — {anion.upper()}$^-$ {T} K", fontsize=11)
    ...
    ax_pairs.set_yticklabels(pair_labels, fontsize=7)
    ax_pairs.set_ylabel("Li$^+$ pair", fontsize=9)
    ...
    ax_pairs.legend(handles=[bridge_patch, no_patch], loc="upper right",
    ...
    ax_count.set_xlabel("Time (ps)", fontsize=9)
