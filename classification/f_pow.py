...
survival times and write fit parameters (β ± SE).

Outputs:
  hist/{anion}/{cat}/{T}_pow.txt            - binned histogram for plotting
  ../../results/table2_f_powerlaw.csv       - β ± SE (→ Table 2, f columns)
...
OUT_CSV = os.path.join(RESULTS_DIR, "table2_f_powerlaw.csv")
if not os.path.exists(OUT_CSV):
    with open(OUT_CSV, "w") as f:
        f.write("anion,T,category,beta,beta_se\n")
